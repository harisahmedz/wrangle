...
| B42 | Optimistic comments/checklist/labels mutations | cycle 4 | ✅ shipped (useOptimistic, tier-1 per UI-UX §6.1) |
...
| B45 | 🐛 Receipts unviewable: `receipt_url` never written, edit drops receipts | cycle 5 audit | ✅ fixed (B45) |
| B46 | Error boundaries: `error.tsx` + `not-found.tsx` + `global-error.tsx` (zero exist) | cycle 5 audit | ✅ shipped |
| B47 | Board route `loading.tsx` (heaviest page has no skeleton) | cycle 5 audit | ✅ shipped |
| B48 | `/boards` real hub — placeholder copy is live in primary nav | cycle 5 audit | ✅ shipped (counts + last activity + board links) |
| B49 | User settings page: theme persist + system option, timezone, currency, avatar | cycle 5 audit | ✅ shipped (`/settings`; avatar deferred — provider-sourced) |
| B50 | Dialog a11y: focus trap + scroll lock + `inert` (fix once in `ui/dialog`) | cycle 5 audit | ✅ shipped |
| B51 | Replace native `confirm()` in members-table with Modal | cycle 5 audit | ✅ shipped |
| B52 | Expense category management UI (actions exist, unreachable) | cycle 5 audit | ✅ shipped (cycle-5b polish) |
| B53 | 🐛 Learn uses UTC instead of `users.timezone` | cycle 5 audit | ✅ fixed (B53) |
| B54 | Filter state in URL (shareable, survives nav; unlocks B18) | cycle 5 audit | ✅ shipped (`?q/label/assignee/due`, `?card=` coexists) |
| B55 | Column drag-reorder + project drag-reorder | cycle 5 audit | ✅ shipped (dnd-kit sortable columns w/ grip handle + sidebar drag; ▲▼ kept; `reorderById` shared helper) |
| B56 | **Weekly Review** screen (promotes B40) — flagship candidate | FEATURES §3.1 | ✅ shipped (`/weekly`: Shipped / Slipped + reschedule-all-to-next-week / Learned / Spent vs last week / Captured + one-tap promote; `W` hotkey, Shutdown link, week nav via `?w=`) |
...
| B63 | README rewrite (still create-next-app boilerplate) | cycle 5 audit | ✅ shipped |
| B64 | 🐛 Migration journal drift: 3 hand-written SQL files outside drizzle journal, colliding prefixes | cycle 5 audit | ✅ fixed — `1xxx_supplement_*` namespace, runtime journal hashes synced |
| B65 | Icon system: lucide-react, kill glyph-buttons, `next/image` avatars | cycle 5 audit | next (G5-04) |
| B66 | `--success`/`--warning` tokens, one palette source, wire motion tokens | cycle 5 audit | ✅ shipped (B66a tokens; B66b `lib/palette.ts` — labels/categories/projects/columns/covers all derive from one 8-hue source) |
...
| 2026-08-25 | 7 | **Phase 3.5 UX hardening completed** (UI-UX §9 audit) via four parallel agents + foundation pass. Foundation: `--success`/`--warning` tokens both themes; migration journal reconciled — strays renamed to `1xxx_supplement_*`, `drizzle_migrations` hashes updated live, migrate re-verified idempotent; `0006_loop_foundation` applied to Neon (`node --env-file=.env.local scripts/migrate.mjs`). **P0:** receipts viewable + preserved on edit + Cloudinary delivery-URL builder incl. pdf/raw routing (B45); `global-error.tsx` house-style boundary (B46); learn dates on `users.timezone` with server-side todayKey backstop (B53). **P1:** `/settings` — theme dark/light/**system** persisted to `users.theme` with live OS-following matchMedia in the init script, curated IANA timezone picker ("Use detected" shortcut), read-only USD row, account echo; user-menu entry (B49). `/boards` real hub — per-project board links, grouped open-counts and last-activity in ≤3 queries (B48). Dialog a11y fixed once: focus trap + scrollbar-compensated scroll lock + inert sibling branches w/ refcounted restore; latest-onClose ref pattern (B50). Last native `confirm()` → Modal (remove member / transfer ownership) (B51). Tier-1 optimistic mutations via `useOptimistic`: checklist tick, label toggle, assignee toggle, comment send with temp-id reconciliation + pending guards (B42/G5-05). Filters moved to URL `?q=&label=&assignee=&due=` with router.replace, `?card=` coexistence preserved (B54). README rewritten for Wrangle incl. migration namespace contract (B63). Verification: typecheck ✓ eslint ✓ vitest **84/84** ✓. Open from audit: palette merge (B66b), lucide icon pass (B65), column drag-reorder (B55). Parking lot created (`docs/PARKING-LOT.md`) for non-Wrangle inspiration (Vibe Kanban). |
| 2026-08-25 | 8 | **New-docs implementation sweep completed** — audited FEATURES/FLAGSHIP/UI-UX against code, then closed everything still open. **Weekly Review (B56):** `/weekly` — tz-aware Mon–Sun windows (`lib/weekly/stats.ts`, pure), Shipped grouped by project, Slipped (went overdue this week) with one-tap **reschedule all to next week** (time-of-day preserving), Learned hours + top item, Spent vs last week (dominant currency) + top category, Captured ideas with one-tap promote of highest score; week nav `?w=`, `W` hotkey, Shutdown-page link. **Palette merge (B66b):** `lib/palette.ts` — one 8-hue source behind PROJECT/LABEL/CATEGORY/COLUMN colors + cover swatches + ACCENT_HEX fallback; zero hardcoded hexes left in components. **Drag-reorder (B55):** generic `reorderById` fractional-index helper (pure, tested); board columns drag via grip handle in dnd-kit horizontal SortableContext with optimistic move + admin-gated `reorderColumn`; project sidebar rows drag too (new `reorderProject`), ▲▼ buttons kept. **Schema cleanup:** cycle-5 "schema with no UI" row was stale — wip_limit/is_collapsed/column color/covers all gained UI; dropped only dead `cards.cover_image_public_id` (`1005_supplement_drop_cover_image.sql`, applied to Neon). B65 lucide pass deliberately deferred. Verification: typecheck ✓ eslint clean ✓ vitest **104/104** (+20 new unit tests across weekly stats ×15 and reorderById ×5) ✓ playwright e2e ✓ production build ✓.
